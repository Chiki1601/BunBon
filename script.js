jQuery(document).ready(function($) {
	...
	var bunBon = ['#head_container', '#body', '#arm-left', '#arm-right', '#foot-left', '#foot-right'];
	...
	function resetFeatures() {
		...
		Object.keys(bodyResetObj).forEach(function (key) {
			...
			TweenMax.to(elem, 0.25, { x: _obj.x, y: _obj.y, opacity: _alpha, rotation: 0, ease: Power4.easeOut });
		});
	}
	...
	function initBunBon() {
		...
		TweenMax.to(bunBon, 0.5, {
			...
			onComplete: function(tween) {
				...
				TweenMax.to(['#ear-left-up-slight', '#ear-right-up-slight'], 0.1, { opacity: 1 });
				
				//	Head
				
				TweenMax.to('#head_container', 0.3, { y: '+=4', ease: Power3.easeOut }).delay(0.2);
				...
				TweenMax.to('#arm-left', 0.3, { rotation: -15, ease: Power4.easeOut }).delay(0.2);
				TweenMax.to('#arm-right', 0.3, { rotation: 15, ease: Power4.easeOut }).delay(0.2);
				...
				TweenMax.to(bunBon, 0.3, {
					...
					onComplete: function(tween) {
						...
						TweenMax.to(['#ear-left-up-slight', '#ear-right-up-slight'], 0.1, { opacity: 0 }).delay(0.1);
						TweenMax.to(['#ear-left', '#ear-right'], 0.1, { opacity: 1 }).delay(0.1);
						
						//	Head
						
						TweenMax.to('#head_container', 0.3, { y: '-=0', ease: Power3.easeOut }).delay(0.2);
						
						//	Face
						
						TweenMax.to('#face', 0.2, { y: 109, ease: Power4.easeOut }).delay(0.1);
						
						//	Arms
						
						TweenMax.to('#arm-left', 0.15, { rotation: 0, ease: Circ.easeOut }).delay(0.2);
						TweenMax.to('#arm-right', 0.15, { rotation: 0, ease: Circ.easeOut }).delay(0.2);
	...
	function bunBlink() {
		...
		(function blinkRepeat() {
			if (blinkCount < 4) {
				...
				TweenMax.to(['#eye-left', '#eye-right'], 0, { opacity: oppositeOpacity });
				TweenMax.to(['#eye-left-closed', '#eye-right-closed'], 0, { opacity: blinkOpacity });
	...
	function lookAround() {
		...
		TweenMax.to('#eye-left', 0.3, { x: 3, y: 0, ease: Power4.easeOut }).delay(1);
		TweenMax.to('#eye-right', 0.3, { x: 53, y: 0, ease: Power4.easeOut }).delay(1);
		
		TweenMax.to('#ear-left', 0.1, { opacity: 1 }).delay(1.9);
		TweenMax.to('#ear-left-up-slight', 0.1, { opacity: 0 }).delay(1.9);
		
		TweenMax.to('#head_container', 0.4, { rotation: 0, ease: Circ.easeOut }).delay(2);
		
		TweenMax.to('#eye-left', 0.3, { x: 0, y: 3, ease: Power4.easeOut }).delay(2);
		TweenMax.to('#eye-right', 0.3, { x: 50, y: 3, ease: Power4.easeOut, onComplete: function() { canBlink(true); } }).delay(2);
	}
	...
	function shrug() {
		...
		TweenMax.to('#arm-left', 0.6, { y: 138, rotation: 15, ease: Power4.easeOut });
		TweenMax.to('#arm-right', 0.6, { y: 138, rotation: -17, ease: Power4.easeOut });
		...
		TweenMax.to('#head_container', 0.6, { 
			...
			onComplete: function(tween) {
				...
				TweenMax.to('#head_container', 0.3, { y: 0, ease: Circ.easeOut }).delay(0.5);
				
				TweenMax.to('#arm-left', 0.3, { y: 145, rotation: 0, ease: Power4.easeOut }).delay(0.5);
				TweenMax.to('#arm-right', 0.3, { y: 145, rotation: 0, ease: Power4.easeOut }).delay(0.5);
	...
	function changeExpression(mood) {
		...
		switch (mood) {
			case 'suspicious' : {
				TweenMax.set(['#eye-left-squint', '#eye-right-squint'], { opacity: 1 });
				TweenMax.set(['#ear-left-up-slight', '#ear-right-down'], { opacity: 1 });
			...
			case 'question' : {
				TweenMax.set(['#eye-left-squint', '#eye-right', '#eyebrow-left', '#eyebrow-right'], { opacity: 1 });
				TweenMax.set(['#ear-left-up-full', '#ear-right-down'], { opacity: 1 });
				TweenMax.to('#head_container', 0.4, { rotation: 4, ease: Power4.easeOut });
				TweenMax.to('#face', 0.4, { y: 106, ease: Power4.easeOut });
			...
			case 'okay' : {
				...
				TweenMax.set(['#ear-left-up-slight', '#ear-right-up-slight'], { y: 32, opacity: 1 });
				TweenMax.to('#head_container', 0.4, { y: -2, ease: Power4.easeOut });
				
				TweenMax.to('#face', 0.4, { y: 100, ease: Power4.easeOut, onComplete: function() {
					//bodyResetObj['face'].x
					
					TweenMax.to('#head_container', 0.25, { y: 3, ease: Power4.easeOut });
					TweenMax.to('#face', 0.25, { y: 120, ease: Power4.easeOut });
					
					TweenMax.set(['#ear-left-up-slight', '#ear-right-up-slight'], { opacity: 0 });
					TweenMax.set(['#ear-left-down', '#ear-right-down'], { y: 46, opacity: 1 });
			...
			case 'normal' :
			default : {
				TweenMax.to('#face', 0.4, { y: 106, ease: Power4.easeOut, onComplete: function() {
					//setTimeout(function() {
						//hideFeatures();
						
						TweenMax.set(['#eye-left', '#eye-right'], { opacity: 1 });
						TweenMax.set(['#ear-left', '#ear-right'], { opacity: 1 });
						TweenMax.to('#head_container', 0.25, { y: 0, ease: Power4.easeOut });
	...
	function changeMood(mood) {
		...
		TweenMax.to('#bgGradient .top-offset', 0.3, { attr: { 'stop-color': topColor }, ease: Power4.easeOut });
		TweenMax.to('#bgGradient .bottom-offset', 0.3, { attr: { 'stop-color': bottomColor }, ease: Power4.easeOut });
	}
	...
	function showXY() {
		Object.keys(bodyResetObj).forEach(function (key) {
			...
			var objString = "'" + key + "'" + ': { x: ' + _mtx[4] + ', y: ' + Math.round(_mtx[5]) + ' }';
